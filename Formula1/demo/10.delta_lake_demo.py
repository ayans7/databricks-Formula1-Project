...
# MAGIC location '/mnt/formula1datal7/demo'

# COMMAND ----------

race_results_df = spark.read.option("inferSchema",True).\
  parquet("/mnt/formula1datal7/presentation/race_results")
...
race_results_df.limit(100).write.format("delta").save("/mnt/formula1datal7/demo/race_results_external")

# COMMAND ----------

# MAGIC %sql
# MAGIC --Making a table out of an external location and type external table as we saved it at a location
# MAGIC
# MAGIC USE CATALOG hive_metastore;
# MAGIC create table f1_demo.race_results_external
# MAGIC using delta
# MAGIC location '/mnt/formula1datal7/demo/race_results_external';
...
race_results_ext_df = spark.read.load("/mnt/formula1datal7/demo/race_results_external", format="delta")
...
deltaTable = DeltaTable.forPath(spark, '/mnt/formula1datal7/demo/race_results_managed')
...
drivers_1_df = spark.read.option("inferSchema",True).\
  json("/mnt/formula1datal7/raw/drivers.json")\
      .filter("driverId <= 10")\
...
delta_table = DeltaTable.forPath(spark, '/mnt/formula1datal7/demo/drivers_merge')
...
# df = spark.read.option("timestampAsOf", '2025-04-17T10:56:54.000+00:00').load("/mnt/formula1datal7/demo/drivers_merge", format="delta")
df = spark.read.option("versionAsOf", '2').load("/mnt/formula1datal7/demo/drivers_merge", format="delta")
display(df)
